...
이 프로젝트의 eBPF 프로그램이 정상적으로 로드되고 동작하기 위해 다음 조건이 필요합니다.

*   **Linux Kernel 5.8 이상**: `BPF_MAP_TYPE_RINGBUF` 기능을 사용하기 위해 필요합니다. (EKS 5.10, 6.12 등 최신 배포판 권장)
*   **BTF (BPF Type Format) 지원**: `/sys/kernel/btf/vmlinux` 파일이 존재해야 합니다. (CO-RE 기능의 핵심)
*   **BPF Subprograms 지원**: `__noinline` 기능을 통한 검증기 부하 분산을 위해 커널 5.3 이상이 필요합니다. (본 프로젝트의 최소 사양인 5.8에 포함됨)
